      ...
      <div className="flex flex-col items-center gap-12 w-full sm:w-[360px] min-h-screen py-4 bg-main-medium">
        ...
        <div className="flex flex-col items-center w-full">
          {MENU.map((item, index) => {
            return (
              ...
              >
                ...
                {pathname.startsWith(item.src) && (
                  <div className="absolute top-0 left-0 w-1 h-full bg-main-bright"></div>
                )}
              </Link>
            )
          })}
          <div
            ...
            className={cn(
              'relative flex items-center justify-center gap-2 w-full h-16 text-white text-opacity-40 cursor-pointer',
            )}
          >
        ...
        {isModal && (
          <ModalPortal>
            <ConfirmModal setState={setIsModal} value={{ yes: '로그아웃' }} handler={handleLogout}>
              <p>로그아웃 하시겠습니까?</p>
            </ConfirmModal>
          </ModalPortal>
        )}
